          ...
          {userImages.length === 0 && !isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {userImages.map((image) => (
                <div 
                  key={image.id} 
                  className="group relative aspect-[3/4] rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300"
                >
                  {imageUrls[image.id] ? (
                    <>
                      <img
                        src={imageUrls[image.id]} // Use the URL from state
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gray-200">
                      ...
                      {/* Debug info for why URL might be missing */}
